...
/** Parse a JSONL file starting from a specific byte offset. */
export async function parseSessionJSONLIncremental(
  filePath: string,
  startOffset: number,
  isSubagent = false
): Promise<{ calls: ApiCall[]; bytesRead: number }> {
  if (!fs.existsSync(filePath)) return { calls: [], bytesRead: 0 };

  const stat = fs.statSync(filePath);
  if (stat.size <= startOffset) return { calls: [], bytesRead: 0 };
  ...
  for await (const line of rl) {
    // Skip the first partial line if starting from an offset
    if (firstLine) {
      firstLine = false;
  ...
  return {
    ...
    bytesRead: stat.size - startOffset,
  };
}
